    ...
    public class Hand : MonoBehaviour
    {
        ...
        public GameObject currentAttachedObject
        {
            get
            {
                ...
                {
                    /*
                     * This section handles pointer detachment from Node GameObjects on pickup
                     * The fixed joint used to connect the two objects needs to be destroyed, 
                     * but only if the object has the tag "PointerType" because we want to be able to
                     * move the nodes around with all of the pointers attached by default.
                     */
                    GameObject obj = attachedObjects[attachedObjects.Count - 1].attachedObject;
                    if (obj.tag == "PointerType") //check tag for pointer type
                    {
                        FixedJoint fj = obj.GetComponent<FixedJoint>(); //find any FixedJoint component on pointer
                        Destroy(fj); //Destroy fixedjoint
                    }
                    return obj;
                }
